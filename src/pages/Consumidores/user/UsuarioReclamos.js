      ...
      <div className="flex flex-col gap-y-5 reclamos overflow-y-auto">
        {
          reclamos.map(r => (
            <Link key={r.id} to={`/comprador/usuarios/3/reclamos/${r.id}`}>
              <p className="flex justify-between text-color_green_6">{r.descripcion}<div className="flex items-center gap-x-3"><span>{r.fecha}</span><span className="text-color_green_5"><IconLink /></span></div></p>
            </Link>
          ))
        }
