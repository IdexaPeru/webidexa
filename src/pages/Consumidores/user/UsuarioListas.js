      ...
      <div className="listas overflow-y-auto flex flex-col gap-y-5">
        {listas.map(l => (
          <Link key={l.id} to={`/comprador/usuarios/3/listas/${l.id}`}>
            <p className="flex justify-between text-color_green_6">{l.nombre}<div className="flex items-center gap-x-3"><span>cantidad {l.cantidad}</span><span className="text-color_green_5"><IconLink /></span></div></p>
          </Link>
        ))}
