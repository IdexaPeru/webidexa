      ...
      <div className="direcciones overflow-y-auto flex flex-col gap-y-5">
        {
          direcciones.map(d => (
            <Link to={`/comprador/usuarios/3/direcciones/${d.id}`} key={d.id} >
              <p className="flex justify-between text-color_green_6">{d.nombre}<div className="flex items-center gap-x-3"><span>{d.provincia}</span><span className="text-color_green_5"><IconLink /></span></div></p>
            </Link>
          ))
        }
