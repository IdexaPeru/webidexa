...

        {
          ...
          <div className="w-full absolute p-2 bg-color_green_1  bottom-0  flex justify-center">
            <div className=" py-3 w-11/12 left-0 text-color_green_3 flex justify-around font-semibold font-poppins   ">
              <NavLink
                to='/comprador/clientes' className={({ isActive }) => (`transition ease-in duration-300 ${isActive ? ' text-color_green_9' : 'text-color_green_5'}`)}

              >
                {({ isActive }) => (
                  isActive ? <IconUsers /> : <IconUsersInactive />
                )}

              </NavLink>
              <NavLink to='/pedidos' className={({ isActive }) => (`transition ease-in duration-300 ${isActive ? ' text-color_green_9' : 'text-color_green_5'}`)} >
                {({ isActive }) => (
                  isActive ? <IconCardBuy /> : <IconCardBuyInactive />
                )}
              </NavLink>
              <NavLink to='/reportes' className={({ isActive }) => (`transition ease-in duration-300 ${isActive ? ' text-color_green_9' : 'text-color_green_5'}`)} >

                {({ isActive }) => (
                  isActive ? <IconEstadistica /> : <IconEstadisticaInactivo />
                )}
              </NavLink>
              ...
              <NavLink to='/configuracion' className={({ isActive }) => (`transition ease-in duration-300 ${isActive ? ' text-color_green_9' : 'text-color_green_5'}`)} >
                {({ isActive }) => (
                  isActive ? <IconConfig /> : <IconConfigInactivo />
                )}
              </NavLink>
            </div>
          </div>
